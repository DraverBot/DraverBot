...
export default new AmethystCommand({
    ...
    if (user.bot) return interaction.reply({
        embeds: [ basicEmbed(interaction.user)
            ...
            .setDescription(`Vous ne pouvez pas envoyer de l'argent à un robot${random({ max: 100 }) === 14 ? ' (il ne saurait pas quoi en faire)':''}`)
            .setColor(evokerColor(interaction.guild))
        ]
    }).catch(() => {});

    if (interaction.client.coinsManager.getData({
        guild_id: interaction.guild.id,
        user_id: interaction.user.id
    }).coins < amount) return interaction.reply({
    ...
    const validated = await confirm({
        ...
        embed: basicEmbed(interaction.user)
            ...
            .setDescription(`Vous êtes sur le point de donner **${numerize(amount)} ${util('coins')}** à ${user}.\nVoulez-vous continuer ?`)
    })
