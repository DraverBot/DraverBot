    ...
    if (sub === 'voir') {
        interaction.reply({
            embeds: [ basicEmbed(interaction.user, { defaultColor: true })
                ...
                .setDescription(`Vous possédez **${numerize(data.bank)} ${util('coins')}** sur votre compte, soit **${Math.floor(data.bank * 100 / (data.bank + data.coins))}%** de votre argent total`)
            ]
        }).catch(() => {});
    }
    if (sub === 'déposer') {
        const amount = options.getInteger('montant');
        if (data.coins < amount) return interaction.reply({
        ...
        interaction.reply({
            embeds: [ basicEmbed(interaction.user, { defaultColor: true })
                ...
                .setDescription(`${numerize(amount)} ${util('coins')} ${plurial(amount, { singular: "a été déposé", plurial: "ont été déposés" })} sur votre compte en banque`)
            ]
        }).catch(() => {});
    }
    if (sub === 'retirer') {
        const amount = options.getInteger('montant');
        if (data.bank < amount) return interaction.reply({
            embeds: [replies.notEnoughCoins(interaction.member as GuildMember)]
        }).catch(() => {});
        ...
        interaction.reply({
            embeds: [ basicEmbed(interaction.user, { defaultColor: true })
                .setTitle("Transaction effectuée")
                .setDescription(`${numerize(amount)} ${util('coins')} ${plurial(amount, { singular: "a été retiré", plurial: "ont été retirés" })} de votre compte en banque`)
            ]
        }).catch(() => {});
    }
})