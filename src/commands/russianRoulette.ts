...
.setChatInputRun(async({ interaction, options }) => {
    ...
    if (kick && !interaction.guild) return interaction.reply({
        embeds: [ replies.guildOnly(interaction.user, { guild: interaction?.guild }).addFields({ name: '⚠️ option', value: `Vous ne pouvez pas utiliser cette commande en messages privés, car l'option d'expulsion n'est valable que dans un serveur.\nSi vous voulez jouer à la roulette russe, n'activez pas l'option \`expulsion\``, inline: false }) ],
    }).catch(() => {});
        ...
        const msg = await interaction.reply({
            embeds: [
                basicEmbed(interaction.user)
                    .setDescription(`Vous êtes sur le point de jouer à la roulette-russe avec la possibilité de vous faire expulser.\nVoulez-vous continuer ?`)
                    .setColor('Grey')
                    .setTitle("Expulsion")
            ],
            components: [ yesNoRow() ],
    ...
    await systemReply(interaction, {
        embeds: [
            basicEmbed(interaction.user)
                ...
                .setDescription(win ? `Vous avez joué et vous avez gagné à la roulette russe${kick ? `\n\n> Vous ne serez pas expulsé`:''}` : `Vous avez perdu à la roulette russe${kick ? '\n\n> Vous serez expulsé dans 5 secondes' :''}`)
