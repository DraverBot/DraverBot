...
import { ComponentType, Message, ModalBuilder, ModalSubmitInteraction, TextInputBuilder, TextInputStyle } from "discord.js";
...
export default new AmethystCommand({
    ...
    const msg = await res.deferReply({
        fetchReply: true
    }).catch(() => {}) as Message<true>;

    const coupon = await query<coupons>(`SELECT * FROM ${DatabaseTables.Coupons} WHERE coupon="${sqliseString(res.fields.getTextInputValue('coupon.code'))}"`);

    if (!coupon || coupon.length === 0) return res.editReply({
    ...
    await res.editReply({
        embeds: [ basicEmbed(interaction.user, { questionMark: true })
            ...
            .setDescription(`Voulez-vous réclamer ce coupon d'une valeur de **${numerize(coupon[0].amount)} ${util('coins')}** ?`)
        ],
        components: [ yesNoRow() ]
    }).catch(() => {});
    ...
    if (!input || input.customId === 'no') return res.editReply({
    ...
    res.editReply({
        embeds: [ basicEmbed(interaction.user, { defaultColor: true })
            ...
            .setDescription(`**${coupon[0].amount} ${util('coins')}** ${coupon[0].amount === 1 ? 'a été ajouté' : 'ont été ajoutés'} à votre compte`)
        ],
        components: []
    }).catch(() => {});
