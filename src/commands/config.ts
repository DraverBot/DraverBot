...
export default new AmethystCommand({
    ...
    if (subcommand === 'paramètre') {
        ...
        if (parameter.type === 'boolean') {
            const reply = await confirm({
                ...
                embed: basicEmbed(interaction.user)
                    .setTitle("État")
                    .setDescription(`Vous allez configurer le paramètre **${parameter.name}**.\nVoulez-vous l'activer ou le désactiver ?`),
            ...
            if (reply === 'cancel' || reply?.value === undefined) return interaction.editReply({
            ...
            const modal = new ModalBuilder()
                ...
                .setComponents(
                    row<TextInputBuilder>(new TextInputBuilder()
                        .setLabel(`Valeur`)
                        .setPlaceholder(`Entrez ici la valeur de ${parameter.name} ( ${parameter.type === 'number' ? 'nombre' : 'texte'} )`)
            ...
            if (parameter.type === 'number') {
                ...
                if (isNaN(test) || test < 0) return interaction.editReply({
            ...
            interaction.client.configsManager.setValue(interaction.guild.id, parameter.value, typeof value === 'boolean' ? value : value.toString());
            reply.reply({
                embeds: [ basicEmbed(interaction.user, { defaultColor: true })
                    .setTitle("Paramètre configuré")
                    .setDescription(`Le paramètre **${parameter.name}** a été configuré sur \`\`\`${value}\`\`\``)
                ],
                components: []
            }).catch(() => {});
            return 
            ...
            if (!reply || reply.content.toLowerCase() === 'cancel') return interaction.editReply({
                embeds: [ replies.cancel() ]
            }).catch(() => {});

            const channel = (reply.mentions.channels.first() ?? interaction.guild.channels.cache.get(reply.content.split(' ')[0]) ?? interaction.guild.channels.cache.find(x => x.name.toLowerCase().includes(reply.content.toLowerCase()))) as BaseChannel;
            ...
            if (!channel) return interaction.editReply({
                embeds: [ basicEmbed(interaction.user)
                    ...
                    .setDescription(`Aucun salon n'a été trouvé, réessayez avec un identifiant, un nom ou une mention`)
                    .setColor(evokerColor(interaction.guild))
                ]
            }).catch(() => {});
            if (channel.type !== ChannelType.GuildText) return interaction.editReply({
                embeds: [ basicEmbed(interaction.user)
                    .setTitle("Salon invalide")
                    .setDescription(`Le salon ${pingChan(channel)} n'est pas un salon textuel.\nOr, le paramètre **${parameter.name}** n'est configurable que sur un salon textuel`)
                    .setColor(evokerColor(interaction.guild))
                ]
            }).catch(() => {});
            ...
            const confirmation = await confirm({
                ...
                embed: basicEmbed(interaction.user)
                    ...
                    .setDescription(`Vous allez configurer le paramètre **${parameter.name}** sur le salon ${pingChan(channel)}`)
            }).catch(() => {}) as confirmReturn;
        ...
        interaction.editReply({
            embeds: [ basicEmbed(interaction.user, { defaultColor: true })
                .setTitle("Paramètre configuré")
                .setDescription(`Le paramètre **${parameter.name}** a été configuré sur ${parameter.type === 'channel' ? pingChan(value as string) : parameter.type === 'boolean' ? value ? 'activé' : 'désactivé' : '```' + value + '```'}`)
            ],
            components: []
        }).catch(() => {});
    };
