...
import { addModLog, basicEmbed, codeBox, evokerColor, hint, numerize, pagination, plurial, random, subcmd } from "../utils/toolbox";
...
export default new AmethystCommand({
    ...
    if (cmd === 'créer') {
        ...
        const res = await query<{ coupon: string }>(`SELECT coupon FROM ${DatabaseTables.Coupons} WHERE coupon IN ( ${tries.map(x => `"${x}"`).join(', ')} )`);
        const valid = tries.filter(x => !res.find(y => y.coupon === x));
        if (valid.length === 0) return interaction.editReply({
            embeds: [ basicEmbed(interaction.user)
                .setTitle("Erreur de génération")
                .setDescription(`Je n'ai pas pu générer de code pour le coupon.\nVeuillez réessayez la commande.\n${hint(`Si l'erreur persiste, contactez le serveur de support`)}`)
                .setColor(evokerColor(interaction.guild))
            ]
        }).catch(() => {});

        await query(`INSERT INTO ${DatabaseTables.Coupons} ( guild_id, coupon, amount ) VALUES ( '${interaction.guild.id}', "${valid[0]}", '${amount}' )`);
        await addModLog({
        ...
        interaction.editReply({
            content: `Le coupon d'une valeur de ${numerize(amount)} ${util('coins')} a été généré. Vous pouvez voir la liste des coupons avec la commande \`/admincoupons liste\`\n\nLe code du coupon est : ${codeBox(valid[0])}`
        }).catch(() => {});
    }
    if (cmd === 'liste') {
        await interaction.deferReply({ ephemeral: true });
        const list = await query<coupons>(`SELECT * FROM ${DatabaseTables.Coupons} WHERE guild_id='${interaction.guild.id}'`);

        if (list.length === 0) return interaction.editReply({
            embeds: [ basicEmbed(interaction.user)
                .setColor(evokerColor(interaction.guild))
    ...
    if (cmd === 'supprimer') {
        ...
        const res = await query<coupons>(`DELETE FROM ${DatabaseTables.Coupons} WHERE coupon="${coupon}" RETURNING amount, coupon`);
        addModLog({
            guild: interaction.guild,
            mod_id: interaction.user.id,
            member_id: '',
        ...
        interaction.editReply({
            embeds: [ basicEmbed(interaction.user, { defaultColor: true })
                ...
                .setDescription(`Un coupon d'une valeur de ${numerize(res[0].amount)} ${util('coins')} a été supprimé`)
                .setFields({
                    name: 'Code',
