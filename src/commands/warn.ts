...
import { addModLog, addProof, basicEmbed, boolDb, codeBox, confirm, dbBool, displayDate, evokerColor, modFields, numerize, pagination, pingUser, plurial, resizeString, sqliseString, subcmd } from "../utils/toolbox";
...
export default new AmethystCommand({
    ...
    if (cmd === 'liste') {
        ...
        const warns = await query<modlogs>(`SELECT * FROM modlogs WHERE guild_id='${interaction.guild.id}' AND member_id='${user.id}' AND type='Warn' AND deleted='${boolDb(false)}'`);

        if (!warns || warns.length === 0) return interaction.editReply({
        ...
        const map = (embed: EmbedBuilder, warn: modlogs) => {
            return embed.addFields({
                ...
                value: `Par ${pingUser(warn.mod_id)} ( \`${warn.mod_id}\` ) ${displayDate(parseInt(warn.date))}\n> Identifiant: \`${warn.case_id}\``,
                inline: false
            })
        }
    ...
    if (cmd === 'ajouter') {
        ...
        if (user.bot) return interaction.reply({
            embeds: [ replies.memberBot(interaction.user, { member: options.getMember('membre') as GuildMember }) ],
            ephemeral: true
        }).catch(() => {});
        ...
        const valid = await confirm({
            embed: addProof(basicEmbed(interaction.user)
                ...
                .setDescription(`Vous êtes sur le point d'avertir ${user} pour la raison :\n${codeBox(reason)}\nVoulez-vous continuer ?`)
            , proof),
            user: interaction.user,
            interaction
        }).catch(() => {}) as confirmReturn;

        if (valid === 'cancel' || !valid?.value) return interaction.editReply({
        ...
        await Promise.all([
            ...
            user.send({
                embeds: [ addProof(basicEmbed(interaction.user)
                    .setTitle("Avertissement")
                    .setDescription(`Vous avez été avertit par ${interaction.user} sur ${interaction.guild.name} <t:${Math.floor(Date.now() / 1000)}:R>`)
                    .setFields({
                        name: 'Raison',
                        value: reason,
                        inline: false
    ...
    if (cmd === 'supprimer') {
        ...
        const warns = await query<modlogs>(`SELECT * FROM modlogs WHERE guild_id='${interaction.guild.id}' AND member_id='${user.id}' AND type='Warn' AND deleted='${boolDb(false)}' AND case_id="${id}"`);
        ...
        await Promise.all([
            ...
            user.send({
                embeds: [ basicEmbed(interaction.user, { defaultColor: true })
                    ...
                    .setDescription(`Votre avertissement d'identifiant \`${id}\` sur ${interaction.guild.name} a été supprimé par ${interaction.user.tag}`)
                ]
            }).catch(() => {})
        ]);
