...
export default new AmethystCommand({
    ...
    const addSigns = (embed: EmbedBuilder, { one, two }: { one: shifumiSign; two: shifumiSign }) => {
        embed.addFields(
            ...
            {
                name: nickname(user ? options.getMember('adversaire') as GuildMember : interaction.guild?.members?.me ?? interaction.client.user),
                value: two.emoji,
                inline: true
            }
        );
    ...
    if (!user) {
        ...
        if (user.bot) return interaction.reply({
            embeds: [ replies.memberBot(interaction.user, { member: options.getMember('adversaire') as GuildMember }) ],
            ephemeral: true
        }).catch(() => {});
        ...
        const msg = await interaction.reply({
            ...
            embeds: [ basicEmbed(interaction.user)
                .setTitle("Défi")
                .setDescription(`${interaction.user} vous défie dans un duel de shifumi.\nAcceptez-vous de relever le défi ?`)
                .setColor('Grey')
            ],
            fetchReply: true,
            components: [ yesNoRow() ]
        ...
        if (!yesNoRep) return interaction.editReply({ embeds: [ replies.cancel() ], components: [], makeContent() {
            return undefined;
        }, }).catch(() => {});
        if (yesNoRep.customId === 'no') return interaction.editReply({
            components: [],
            embeds: [ replies.cancel() ],
            makeContent() {return undefined}
        }).catch(() => {});
        ...
        if (!userSignID) return interaction.editReply({
            embeds: [ replies.cancel() ],
            components: [],
            makeContent() {return undefined}
        }).catch(() => {});
        ...
        if (!twoSign) return interaction.editReply({
            embeds: [ replies.cancel() ],
            components: [],
            makeContent() {
                return undefined
