...
const format = (text: string, message: Message<true>) => {
    ...
    message.mentions.channels.forEach((channel) => {
        text = text.replace(`<#${channel.id}>`, `<span class="mention interactive">#${(channel as { name: string }).name}</span>`);
    });
...
export default function save(messages: Collection<string, Message<true>>, customId: string) {
    ...
    messages.forEach((message) => {
        ...
        fileContent+= `<div class="message group">
            <div class="contents">
                <img src="${message.author.avatarURL({ forceStatic: false, size: 64, extension: 'png' })}" class="avatar">
                <h2 class="header">
                    <span class="username" style="color: ${message.author.id == message.guild.members?.me?.id ? message.guild.members?.me?.displayHexColor : message.member?.displayHexColor ?? '#ffffff'}">${message.member?.nickname ?? message.author.username}</span>${message.author.bot ? `
