...
export class GiveawayManager<DatabaseMode extends databaseMode> {
    ...
    public createGiveaway(input: giveawayInput): Promise<gwT> {
        return new Promise(async (resolve, reject) => {
            ...
            const data: gwT = {
                ...
                reward: input.reward,
                required_invitations: input.required_invitations ?? 0,
                required_level: input.required_level ?? 0
            };
    ...
    private registerParticipation(interaction: ButtonInteraction<'cached'>) {
        ...
        if (this.client.modulesManager.enabled(interaction.guild.id, 'invitations') && gw.required_invitations > 0) {
            ...
            if (stats.total - (stats.fakes + stats.leaves) < gw.required_invitations) {
                return interaction
                    .reply({
                        embeds: [this.embeds.notEnoughInvitations(gw.required_invitations, this.getUrl(gw))],
                        ephemeral: true
                    })
                    .catch(log4js.trace);
            }
        ...
        if (this.client.modulesManager.enabled(interaction.guild.id, 'level') && gw.required_level > 0) {
            ...
            if (stats.level < gw.required_level) {
                return interaction
                    .reply({
                        embeds: [this.embeds.notEnoughLevel(gw.required_level, this.getUrl(gw))],
                        ephemeral: true
                    })
                    .catch(log4js.trace);
            }
    ...
    private toObj(x: any) {
        ...
        ['endsAt', 'required_level', 'required_invitations'].forEach((v) => {
            gw[v] = parseInt(x[v]);
        });
        gw.ended = gw.ended === 1;
