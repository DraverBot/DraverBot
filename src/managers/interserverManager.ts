...
export class InterserverManager {
    ...
    public createInterserver({ guild_id, channel, frequence }: { guild_id: string; channel: TextChannel; frequence?: string }): Promise<replyKey | interserver> {
        return new Promise(async(resolve) => {
            ...
            if (this._cache.find(x => x.guild_id === guild_id && x.frequence === generated)) return resolve('interserverFrequenceAssigned');
            if (!this._cache.find(x => x.frequence === frequence) && ![undefined, null].includes(frequence)) return resolve('interserverUnexistingFrequence');
            ...
            await query(`INSERT INTO interserver (guild_id, channel_id, webhook, frequence) VALUES ('${guild_id}', '${channel.id}', '${webhook.url}', '${generated}')`).catch(() => {});
    ...
    public removeInterserver({ guild_id, channel }: { guild_id: string; channel: TextChannel }): Promise<interserver | replyKey> {
        return new Promise(async(resolve) => {
    ...
    public editFrequence({ guild_id, channel_id, frequence }: { guild_id: string; channel_id: string; frequence: string }): Promise<replyKey | interserver> {
        return new Promise(async(resolve) => {
            const data = this._cache.get(channel_id);
            if (!data) return resolve('interserverNotChannel');

            if (this._cache.find(x => x.guild_id === guild_id && x.frequence === frequence)) return resolve('interserverFrequenceAssigned');

            this._cache.set(data.guild_id, {
                ...data,
                frequence
            });
            await query(`UPDATE interserver SET frequence='${frequence}' WHERE guild_id='${guild_id}' AND channel_id='${channel_id}'`).catch(() => {});
            return resolve(data)
        })
    }
    ...
    private async start() {
        await query(`CREATE TABLE IF NOT EXISTS interserver ( channel_id VARCHAR(255) NOT NULL, frequence VARCHAR(255) NOT NULL, webhook VARCHAR(255) NOT NULL, guild_id VARCHAR(255) NOT NULL )`)
    ...
    private event() {
        this.client.on('messageCreate', (message) => {
            ...
            if (!data || !message.client.modulesManager.enabled(message.guild.id, 'interchat') || message.author.bot || message.webhookId || message.system || message.mentions.everyone || /<@((\!){0,1})\d+>/.test(message.content)) return;

            const sendTo = this._cache.filter(x => x.frequence === data.frequence && message.client.modulesManager.enabled(x.guild_id, 'interchat') && x.guild_id !== message.guild.id);
            ...
            sendTo.forEach(async(v) => {
                ...
                if (!webhook) {
                    ...
                    const web = await channel.createWebhook({
                        name: 'Interchat',
