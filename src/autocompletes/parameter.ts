...
export default new AutocompleteListener({
    ...
    run: ({ focusedValue }) => {
        ...
        return parameters.filter(x => x.name.toLowerCase().includes(focusedValue.toLowerCase()) || focusedValue.toLowerCase().includes(x.name.toLowerCase()) || x.description.includes(focusedValue.toLowerCase())).map(x => ({ name: capitalize(x.name), value: x.value }));
    }
})