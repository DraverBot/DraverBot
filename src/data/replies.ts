...
const replies = {
    ...
    clientMissingPermissions: (user: User, metadata: { permissions?: { missing: PermissionsString[] }}) => {
        ...
        return basic(user)
            ...
            .setDescription(`Je n'ai pas les permissions nécéssaires pour éxécuter cette commande.\n${missing.length === 1 ? `Vérifiez que j'aie bien la permission \`${getPerm(missing[0] as permType)}\`` : `Vérifiez que j'aie bien les permissions : ${missing.map((perm) => `\`${getPerm(perm as permType)}\``).join(' ')}`}`)
            .setColor('#ff0000')
    },
    userMissingPermissions: (user: User, metadata: { permissions?: { missing: PermissionsString[] } }) => {
        ...
        return basic(user)
            .setTitle(":x: Permissions insuffisantes")
            .setDescription(`Vous n'avez pas pas les permissions nécéssaires pour éxécuter cette commande.\n${missing.length === 1 ? `Vérifiez que vous ayez bien la permission \`${getPerm(missing[0] as permType)}\`` : `Vérifiez que vous ayez bien les permissions : ${missing.map((perm) => `\`${getPerm(perm as permType)}\``).join(' ')}`}`)
            .setColor('#ff0000')
    },
